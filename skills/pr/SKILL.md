...
1. Resolve the active repository, worktree, branch, base, issue, and committed head from the request or current context. Verify the worktree head before each review.
...
6. Immediately before publication, verify the worktree `HEAD` still equals the reviewed head; otherwise publish nothing and return to step 1. Use internal mode for the user's repositories and external mode otherwise; ask only when ownership is ambiguous:
