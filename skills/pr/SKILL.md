...
For UI changes, run `gh image --version` and require `gh-image` v1.3.0 or later. When it is available, publish each file with `gh image <file> --repo OWNER/REPO`; otherwise use the GitHub editor. Allow `gh-image` to fall back from the GitHub CLI token to the local browser session when needed. Use each returned reference exactly as emitted: images use Markdown and videos use a bare GitHub attachment URL that renders inline. Replace only local media references with the returned references or editor attachment URLs, and verify no local paths remain.
