...
When Codex reviews the published PR, review the complete PR diff from its base through the current head, never only the latest commit. Submit at most one `COMMENT` review per head with the body exactly `Codex reviewed this PR. No issues found.` Keep head SHAs, pass markers, checks, commands, task state, and other internal details out of the GitHub comment. Never impersonate the user or claim GitHub approval. In orchestrated mode, the caller owns CI monitoring and full-PR review of the final head.
