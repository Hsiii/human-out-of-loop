...
5. Restore one run-level heartbeat on the orchestration task if unattended work remains, updating the existing automation instead of creating a duplicate. Each wake performs one reconciliation and one bounded `wait_threads` snapshot over active workers, then returns control. Delete the heartbeat when all requested issues are terminal.
