...
4. Reconcile each issue by its furthest durable stage:
   ...
   - `done`: verify the final head, review pass, and any required CI result. Stop the heartbeat but leave tasks pinned and unarchived and preserve the worktree and branch. On follow-up changes, wake the existing workers and resume at `fix`.
